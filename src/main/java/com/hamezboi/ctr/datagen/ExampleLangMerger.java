package com.hamezboi.ctr.datagen;
 ...
 * {@code assets/ctr/lang/default/*.json}, and only the translation <em>keys</em>
 * appear in code (see {@link com.hamezboi.ctr.Lang}).
 *
 ...
 * into that same provider via {@link CTR#registerLangPartials()}, so the partials
...
public class ExampleLangMerger {
    ...
    /** Partial file names under {@code assets/ctr/lang/default/}, without ".json". */
    ...
    public static void mergeInto(BiConsumer<String, String> consumer) {
        for (String partial : PARTIALS) {
            String path = "assets/" + CTR.ID + "/lang/default/" + partial + ".json";
            try (InputStream in = ExampleLangMerger.class.getClassLoader().getResourceAsStream(path)) {
                if (in == null) {
                    CTR.LOGGER.error("Lang partial not found on the classpath: {}", path);
                    continue;
                }
